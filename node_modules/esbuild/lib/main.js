...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.8.23") {
        throw new Error(`Cannot start service: Host version "${"0.8.23"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
    ...
    service: {
      buildOrServe(callName, serveOptions, options, isTTY2, callback) {
        ...
        try {
          ...
          let [flags, write, plugins, stdin, resolveDir, incremental] = flagsForBuildOptions(callName, options, isTTY2, logLevelDefault, writeDefault);
...
var startService = (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.8.23"}`), {
    cwd: process.cwd(),
    windowsHide: true,
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.8.23"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
